// c001-alt0: Like c001, but values set so that all frequency buckets are visible.
// Sounds great in theory, but (a) the higher frequencies probably cannot be estimated
// very accurately relatively to the sample rate, and (b) the high frequency values
// tend to be very small so the visuals look extra skewed to the left.
...
constexpr const int DISPLAY_FREQ_BIN_COUNT = 1024;
